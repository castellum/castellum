...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                bool fAllowFree = CTransaction::AllowFree(dPriority); // No free TXs in CASTL
               //bool fAllowFree = false;					// No free TXs in CASTL
...

   {
       LOCK2(cs_main, cs_wallet);
       {
           ...
           loop
           {
               ...
               // Check that enough fee is included
               int64 nPayFee = nTransactionFee * (1 + (int64)nBytes / 1000);
           //    bool fAllowFree = CTransaction::AllowFree(dPriority); // No free TXs in NULL
                bool fAllowFree = false;					// No free TXs in NULL
                int64 nMinFee = wtxNew.GetMinFee(1, fAllowFree, GMF_SEND);
                if (nFeeRet < max(nPayFee, nMinFee))
                {
                    nFeeRet = max(nPayFee, nMinFee);
